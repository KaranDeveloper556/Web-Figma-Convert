...
const DetailsSection = () => {
  return (
    <section className="relative top-[-5rem] Details-section overflow-hidden" id='about'>
      <div className="upload-details flex flex-wrap md:flex-nowrap items-center justify-center gap-2 p-5 mb-[6rem] max-w-[90rem]">
        <motion.div
          className="flex flex-col justify-center gap-[2rem] p-5 datails"
        ...
        <motion.img
          ...
          src={Upload}
          alt="img"
          className='h-[15rem] vsm:h-[18rem] sm:h-[22rem] md:h-[18rem] lg:h-[22rem] xl:h-[25rem] rounded-2xl' />
      </div>
      ...
      <div className="points" id='features'>
        <motion.div
          initial={{ x: 200, opacity: 0 }}
          whileInView={{ x: 0, opacity: 1, transition: { duration: 1 } }}
          className="ml-[2rem] md:ml-[4rem] mb-[3rem] flex items-center justify-between md:gap-[10rem] sm:gap-[4rem] gap-[1rem] heading">
        ...
        <div className="flex flex-wrap flex-row-reverse md:flex-nowrap items-center justify-center gap-2 px-[1rem] lg:px-[4rem] max-w-[90rem]">
          <motion.div
            initial={{ x: -200, opacity: 0 }}
            whileInView={{ x: 0, opacity: 1, transition: { duration: 1 } }}
            className="flex flex-col justify-center gap-[2rem] md:p-2 p-5 datails">
          ...
          <motion.img
            initial={{ x: -200, opacity: 0 }}
            whileInView={{ x: 0, opacity: 1, transition: { duration: 1 } }}
            src={QualityCode} alt="" className='h-[20rem] vsm:h-[15rem] sm:h-[22rem] md:h-[22rem] lg:h-[26rem] xl:h-[34rem]' />
        </div>
      </div>
    </section>
  )
